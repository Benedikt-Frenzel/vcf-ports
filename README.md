...
- Interactive infrastructure diagram inspired by the official VCF 9.1 Fleet Latency diagram, with nested Fleet Services (including VCF Operations HCX), Management Domain (VCF Management Services, VCF Installer, SDDC Manager, vCenter, Software Depot, Identity Broker, License Hub/Server), Workload Infrastructure, Advanced Services, and External Systems boxes. Select a component to draw its direct paths; select a connected box or path to isolate it and label its ports/protocols directly on the link.
- Compact, expandable port groups by direction and protocol, with exact label counts and an expand/collapse-all control.
- Path derivation for every selected component or path: each published label pair shows the alias rule that placed it, including internet-domain and Automation VMSP boundary cases. The same explanation is included in the SVG and PNG export.
...
The environment mapping is processed entirely in the browser: the installer JSON is parsed once for addressing fields (hostnames, FQDNs, IP pools, subnets, DNS resolvers, and NTP servers) and then discarded; only the environment-object-to-address mapping is kept in localStorage and included in exports. DNS, NTP, DHCP, directory, syslog, mail, SNMP, and external storage endpoints retain separate firewall objects instead of inheriting every address from the broad topology node. Passwords and credentials are never extracted, stored, or exported. Firewall templates aggregate the same published entries as the rest of the app — they are a request template for network administrators, not a validated firewall policy.
