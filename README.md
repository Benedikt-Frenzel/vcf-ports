...
- Environment mapping: paste a VCF 9.x installer JSON (the [vcf-91-in-box](https://github.com/lamw/vcf-91-in-box) config format is supported) or fill component addresses manually; hostnames, IP pools, and subnets map to diagram components. Values stay in the browser (localStorage) — credentials in the pasted JSON are never read or stored.
...
The environment mapping is processed entirely in the browser: the installer JSON is parsed once for addressing fields (hostnames, FQDNs, IP pools, subnets) and then discarded; only the component-to-address mapping is kept in localStorage and included in exports. Passwords and credentials are never extracted, stored, or exported. Firewall templates aggregate the same published entries as the rest of the app — they are a request template for network administrators, not a validated firewall policy.
